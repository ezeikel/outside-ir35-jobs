...
//
// ⚠️ COPY of apps/web/lib/tax/takeHome.ts — the web file is the canonical source
// of truth. This is a verbatim duplicate (the module is pure, no web deps) kept
// in mobile because the app has no shared TS-utils package. When tax rates change
// (annual, each 6 April) update BOTH files together, or they will drift. The
// tests (takeHome.test.ts, also copied) guard against silent divergence.
//
 ...
 * the 6 April boundary. To add a year: verify every figure against gov.uk,
 * add a CONSTANTS_<year> block, register it in CONSTANTS, and add a fixture to
 * takeHome.test.ts. Update DEFAULT_TAX_YEAR on 6 April.
 */
...
// Every figure verified against gov.uk for tax year 2026/27 (England / NI).
// Changed vs 2025/26: dividend ordinary + upper rates each rose 2 points
// (8.75%→10.75%, 33.75%→35.75%) from 6 April 2026 (Autumn 2025 Budget); the
// additional rate (39.35%) and everything else are unchanged. The Employment
// Allowance £100k-liability eligibility cap was removed, but a single-director
// PSC is still excluded from the EA regardless, so it doesn't affect the model.
const CONSTANTS_2026_27: TaxConstants = {
  CT_SMALL_RATE: 0.19,
  CT_MAIN_RATE: 0.25,
  CT_LOWER_LIMIT: 50_000,
  CT_UPPER_LIMIT: 250_000,
  ...
  DIVIDEND_ALLOWANCE: 500,
  DIV_BASIC_RATE: 0.1075, // ↑ from 0.0875 (2025/26)
  DIV_HIGHER_RATE: 0.3575, // ↑ from 0.3375 (2025/26)
  DIV_ADDITIONAL_RATE: 0.3935, // unchanged
...
const CONSTANTS: Record<TaxYear, TaxConstants> = {
  ...
  '2026/27': CONSTANTS_2026_27,
};

// The current UK tax year (changes each 6 April). It's now 2026/27.
...
export const DEFAULT_SALARY = CONSTANTS[DEFAULT_TAX_YEAR].OPTIMAL_SALARY;

// Small selectors so the UI can label levers with the current year's figures
// (the dividend allowance, the sensible salary) without re-deriving them.
export const taxConstants = (year: TaxYear = DEFAULT_TAX_YEAR): TaxConstants =>
  CONSTANTS[year];
...
export const TAKE_HOME_CAVEATS: readonly string[] = [
  ...
  'Assumes a single director with no other income, over a full tax year. The default is the common low-salary split (£12,570 salary, all post-tax profit drawn as dividends); adjust the salary and dividend levers to model your own split.',
